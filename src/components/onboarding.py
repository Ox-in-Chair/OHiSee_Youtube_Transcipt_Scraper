...
class OnboardingBanner(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # Icon/emoji
        icon = tk.Label(container, text='🎯', font=('Segoe UI', 24),
        ...
        title = tk.Label(content, text='Welcome to YouTube Transcript Scraper Pro',
        ...
        try_btn = tk.Button(buttons, text='Try Sample', font=FONTS['body'],
                          bg='white', fg=COLORS['primary'], relief='flat',
                          padx=16, pady=6, command=self._try_sample)
        try_btn.pack(side='left', padx=(0, SPACING['xs']))

        learn_btn = tk.Button(buttons, text='Learn More', font=FONTS['body'],
                            bg=COLORS['primary'], fg='white', relief='flat',
                            bd=1, padx=16, pady=6, command=self._learn_more)
        learn_btn.pack(side='left', padx=(0, SPACING['xs']))

        dismiss_btn = tk.Button(buttons, text='✕', font=('Segoe UI', 12),
                              bg=COLORS['primary'], fg='white', relief='flat',
                              padx=8, pady=4, command=self._dismiss)

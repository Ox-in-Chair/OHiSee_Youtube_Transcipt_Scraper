...
class LearningLoop:
    ...
    def __init__(self, data_dir: str = None):
        self.data_dir = data_dir or os.path.join(
            Path.home(), '.youtube_scraper_learning'
        )
    ...
    def record_feedback(self, video_id: str, rating: str,
    ...
    def detect_patterns(self) -> Dict[str, Any]:
        ...
        positive_filters = [
            f['filters'] for f in self.feedback_history
...
class LearningInsightsPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # Header
        tk.Label(self, text='Learning Insights', font=FONTS['h2'],
        ...
        tk.Label(stats_frame, text='📊', font=('Segoe UI', 20),
        ...
        insights_frame.pack(fill='both', expand=True, padx=SPACING['md'],
                           pady=SPACING['sm'])

        tk.Label(insights_frame, text='Detected Patterns:',
                font=FONTS['h3'], bg='white',
                fg=COLORS['text']).pack(anchor='w', pady=SPACING['xs'])
...
class FeedbackWidget(tk.Frame):
    ...
    def _build_ui(self):
        """Build feedback UI."""
        tk.Label(self, text='Was this helpful?', font=FONTS['small'],
                bg='white', fg=COLORS['text_secondary']).pack(side='left',
                                                               padx=SPACING['xs'])

        # Thumbs up
        up_btn = tk.Button(self, text='👍', font=FONTS['body'],
                          bg='white', fg=COLORS['success'],
                          relief='flat', cursor='hand2',
                          command=lambda: self._handle_feedback('positive'))
        up_btn.pack(side='left', padx=2)

        # Thumbs down
        down_btn = tk.Button(self, text='👎', font=FONTS['body'],
                            bg='white', fg=COLORS['error'],
                            relief='flat', cursor='hand2',

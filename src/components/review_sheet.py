...
class ReviewSheet(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # Title
        title = tk.Label(
            self, text="Review Configuration", font=FONTS["h2"], bg=COLORS["bg"], fg=COLORS["text"]
        )
    ...
    def _render_section_header(self, title: str):
        ...
        header = tk.Label(
            self.content_frame, text=title, font=FONTS["h3"], bg="white", fg=COLORS["text"]
        )
        header.pack(anchor="w", padx=SPACING["md"], pady=(SPACING["md"], SPACING["xs"]))
    ...
    def _render_config_item(self, label: str, value: Any, highlight: bool = False):
        ...
        item_frame = tk.Frame(
            ...
            bd=1 if highlight else 0,
            relief="solid" if highlight else "flat",
        )

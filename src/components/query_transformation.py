...
class QueryTransformationView(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # Title
        title = tk.Label(self, text='Query Transformation',
        ...
        bypass_cb = tk.Checkbutton(toggle_frame, text='Bypass AI optimization',
                                   variable=self.bypass_var, font=FONTS['body'],
        ...
        self.flow_frame.pack(fill='both', expand=True, padx=SPACING['md'],
                            pady=SPACING['sm'])
    ...
    def _render_flow(self):
        ...
        # Stage 1: Original
        self._render_stage('Original Query', self.original_query or 'Enter your query...',
                          COLORS['text_secondary'], 0)
        ...
        if not self.bypass_ai:
            ...
            self._render_stage('AI Optimized', self.optimized_query or 'AI will optimize...',
                              COLORS['primary'], 2)
        ...
        # Stage 3: YouTube
        youtube_display = self.youtube_query or self.optimized_query or self.original_query or 'Final YouTube query...'
        self._render_stage('YouTube Search', youtube_display, COLORS['success'], 4 if not self.bypass_ai else 2)
    ...
    def _render_stage(self, label: str, query: str, color: str, grid_row: int):
        ...
        stage_frame.grid(row=grid_row, column=0, sticky='ew', padx=SPACING['md'],
                        pady=SPACING['xs'])

        # Label
        label_widget = tk.Label(stage_frame, text=label, font=FONTS['body'],
                               bg='white', fg=color)
        label_widget.pack(anchor='w')

        # Query display
        query_frame = tk.Frame(stage_frame, bg=COLORS['surface'],
                              bd=1, relief='solid')
        query_frame.pack(fill='x', pady=(4, 0))

        query_text = tk.Text(query_frame, height=2, wrap='word',
        ...
        # Copy button
        copy_btn = tk.Button(query_frame, text='📋 Copy', font=FONTS['small'],
    ...
    def _render_arrow(self, grid_row: int):
        ...
        canvas = tk.Canvas(arrow_frame, bg='white', height=24,
                          highlightthickness=0)
        canvas.pack(fill='x', padx=SPACING['lg'])

        # Draw arrow line
        canvas.create_line(20, 12, 200, 12, fill=COLORS['border'],
                          width=2, arrow='last')

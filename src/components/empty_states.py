...
class EmptyState(tk.Frame):
    ...
    def __init__(self, parent, icon: str, title: str, message: str,
    ...
    def _build_ui(self):
        ...
        # Icon
        icon_label = tk.Label(container, text=self.icon,
                             font=('Segoe UI', 64), bg='white',
                             fg=COLORS['text_secondary'])
        icon_label.pack(pady=SPACING['md'])

        # Title
        title_label = tk.Label(container, text=self.title,
                              font=FONTS['h2'], bg='white',
                              fg=COLORS['text'])
        ...
        if self.action_text and self.action_callback:
            action_btn = tk.Button(container, text=self.action_text,
...
class LoadingState(tk.Frame):
    ...
    def _build_ui(self):
        ...
        spinner_label = tk.Label(container, text='⏳',
                                font=('Segoe UI', 64), bg='white')
        spinner_label.pack(pady=SPACING['md'])

        # Message
        self.message_label = tk.Label(container, text=self.message,

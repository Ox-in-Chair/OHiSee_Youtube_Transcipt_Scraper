...
class AITransparencyPanel(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # Toggle button
        self.toggle_btn = tk.Button(header_frame, text='▶ AI Decision Breakdown',
        ...
        # Confidence badge
        self.confidence_label = tk.Label(header_frame, text='85% confidence',
    ...
    def _render_content(self):
        ...
        tk.Label(model_frame, text='Model:', font=FONTS['body'],
                bg='white', fg=COLORS['text_secondary']).pack(side='left')
        tk.Label(model_frame, text=self.current_data.get('model', 'GPT-4'),
                font=FONTS['body'], bg='white', fg=COLORS['text']).pack(side='left', padx=SPACING['xs'])

        # Confidence factors
        factors_label = tk.Label(self.content_frame, text='Confidence Factors',
        ...
        opt_label = tk.Label(self.content_frame, text='Optimizations Applied',
                            font=FONTS['h3'], bg='white', fg=COLORS['text'])
        ...
        for opt in self.current_data.get('optimizations', []):
            ...
            tk.Label(opt_item, text='•', font=FONTS['body'],
                    bg='white', fg=COLORS['primary']).pack(side='left')
            tk.Label(opt_item, text=opt, font=FONTS['body'],
                    bg='white', fg=COLORS['text']).pack(side='left', padx=SPACING['xs'])
    ...
    def _render_factor(self, factor: Dict[str, Any]):
        ...
        tk.Label(info_frame, text=factor['name'], font=FONTS['body'],
                bg='white', fg=COLORS['text']).pack(side='left')

        score_label = tk.Label(info_frame, text=f"{factor['score']}%",
        ...
        tk.Label(info_frame, text=factor.get('impact', 'medium').upper(),
                font=FONTS['small'], bg=impact_bg, fg='white',
                padx=6, pady=2).pack(side='right', padx=SPACING['xs'])
    ...
    def show_optimization_details(self, original: str, optimized: str, reasoning: str):
        ...
        detail_frame = tk.Frame(self.content_frame, bg=COLORS['surface'],
                               bd=1, relief='solid')
        detail_frame.pack(fill='x', padx=SPACING['md'], pady=SPACING['sm'])

        tk.Label(detail_frame, text='Query Transformation',
                font=FONTS['h3'], bg=COLORS['surface'],
                fg=COLORS['text']).pack(anchor='w', padx=SPACING['sm'],
                                       pady=(SPACING['xs'], 0))

        tk.Label(detail_frame, text=f"Original: {original}",
                font=FONTS['body'], bg=COLORS['surface'],
                fg=COLORS['text_secondary'], wraplength=400,
                justify='left').pack(anchor='w', padx=SPACING['sm'], pady=2)

        tk.Label(detail_frame, text=f"Optimized: {optimized}",
                font=FONTS['body'], bg=COLORS['surface'],
                fg=COLORS['text'], wraplength=400,
                justify='left').pack(anchor='w', padx=SPACING['sm'], pady=2)

        tk.Label(detail_frame, text=f"Why: {reasoning}",

...
class AccessibilityHelper:
    ...
    def add_focus_indicators(widget: tk.Widget, color: str = None):
        ...
        def on_focus_in(event):
            widget.configure(highlightbackground=focus_color, highlightthickness=2)
...
class SkipNavigation(tk.Frame):
    ...
    def _build_ui(self):
        ...
        skip_label = tk.Label(
            self, text="Skip to:", font=FONTS["body"], bg=COLORS["primary"], fg="white"
        )
        ...
        for name, widget in self.targets.items():
            ...
            # Show on focus
            btn.bind(
                "<FocusIn>", lambda e: self.pack(fill="x", before=self.master.winfo_children()[0])
            )
...
class AccessibleButton(tk.Button):
    ...
    def __init__(self, parent, text: str, command: Callable, aria_label: str = None, **kwargs):

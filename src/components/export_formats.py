    ...
    def export_csv(transcripts: List[Dict[str, Any]], filepath: str) -> bool:
        ...
        try:
            with open(filepath, 'w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=['title', 'channel', 'url', 'upload_date', 'transcript'])
                writer.writeheader()
                for transcript in transcripts:
                    writer.writerow({
...
class ExportPanel(tk.Frame):
    ...
    def _build_ui(self):
        """Build export UI."""
        # Header
        tk.Label(self, text='Export Formats', font=FONTS['h2'],
        ...
        for format_name in self.EXPORT_FORMATS.keys():
            rb = tk.Radiobutton(format_frame, text=format_name,
                               ...
                               value=format_name, font=FONTS['body'],
                               bg='white')
            rb.pack(anchor='w', pady=4)

        # Export button
        tk.Button(self, text='Export Transcripts',
    ...
    def _handle_export(self):
        ...
        if format_id == 'obsidian':
            ...
            path = filedialog.asksaveasfilename(
                ...
                filetypes=[(format_name, f'*{ext}'), ('All Files', '*.*')]
            )
        ...
        if path:
            ...
            if success:
                tk.messagebox.showinfo('Export Successful',
                                      f'Exported {len(transcripts)} transcripts')

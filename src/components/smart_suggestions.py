...
class SmartSuggestions:
    ...
    def get_query_suggestions(self, partial_query: str) -> List[Dict[str, str]]:
        ...
        if len(partial_query) < 5:
            suggestions.append({
                'text': 'Try adding more specific keywords',
                'type': 'tip',
                'action': None
            })
        elif not any(op in partial_query for op in ['"', 'OR', '-']):
            suggestions.append({
                'text': 'Use "exact phrases" for precise matches',
                'type': 'tip',
                'action': None
            })
...
class SuggestionPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text='💡', font=('Segoe UI', 16),
                bg=COLORS['surface']).pack(side='left', padx=SPACING['xs'])

        tk.Label(header, text='Smart Suggestions',
    ...
    def _render_suggestion(self, text: str, suggestion_type: str):
        ...
        sugg_frame = tk.Frame(self.suggestions_frame, bg='white',
                             bd=1, relief='solid')
        ...
        tk.Label(sugg_frame, text=icon, font=FONTS['body'],
                bg='white').pack(side='left', padx=SPACING['xs'])

        # Suggestion text
        tk.Label(sugg_frame, text=text, font=FONTS['small'],

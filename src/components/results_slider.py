...
class ResultsSlider(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # Title
        title = tk.Label(
            self, text="Number of Results", font=FONTS["h3"], bg=COLORS["bg"], fg=COLORS["text"]
        )
        ...
        for preset in self.PRESETS:
            btn = tk.Button(
                presets_frame,
                text=f"{preset['label']} ({preset['value']})",
                font=FONTS["body"],
                bg=COLORS["surface"],
                fg=COLORS["text"],
                relief="flat",
        ...
        self.value_label = tk.Label(
            slider_frame, text="15 videos", font=FONTS["h2"], bg=COLORS["bg"], fg=COLORS["primary"]
        )

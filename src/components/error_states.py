...
class ErrorState(tk.Frame):
    ...
    def __init__(
        self, parent, error_type: str, title: str, message: str, recovery_actions: List[dict] = None
    ):
        super().__init__(
            parent,
            bg="white",
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="❌", font=("Segoe UI", 24), bg=COLORS["error"], fg="white").pack(
            side="left", padx=SPACING["sm"], pady=SPACING["xs"]
        )

        tk.Label(header, text=self.title, font=FONTS["h3"], bg=COLORS["error"], fg="white").pack(
            side="left", pady=SPACING["xs"]
        )

        # Error message
        message_frame = tk.Frame(self, bg="white")
        message_frame.pack(fill="both", expand=True, padx=SPACING["md"], pady=SPACING["md"])
        ...
        if self.recovery_actions:
            ...
            for action in self.recovery_actions:
                ...
                tk.Label(
                    action_frame, text="•", font=FONTS["body"], bg="white", fg=COLORS["primary"]
                ).pack(side="left")
...
class InlineErrorMessage(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(container, text="⚠️", font=FONTS["body"], bg="#FEE2E2", fg=COLORS["error"]).pack(
            side="left", padx=SPACING["xs"], pady=4
        )
        ...
        tk.Label(
            ...
            wraplength=400,
            justify="left",
        ).pack(side="left", padx=(0, SPACING["xs"]), pady=4)

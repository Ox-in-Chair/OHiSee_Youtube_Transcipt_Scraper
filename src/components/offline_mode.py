...
class OfflineCache:
    ...
    def __init__(self, cache_dir: str = None):
        self.cache_dir = cache_dir or os.path.join(Path.home(), ".youtube_scraper_cache")
...
class OfflineModePanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # Header
        header = tk.Label(
            self, text="Offline Mode", font=FONTS["h2"], bg="white", fg=COLORS["text"]
        )
        header.pack(pady=SPACING["md"], anchor="w", padx=SPACING["md"])

        # Status indicator
        stats = self.cache.get_cache_stats()
        status_frame = tk.Frame(self, bg=COLORS["surface"], bd=1, relief="solid")
        status_frame.pack(fill="x", padx=SPACING["md"], pady=SPACING["sm"])

        tk.Label(status_frame, text="📦", font=("Segoe UI", 20), bg=COLORS["surface"]).pack(

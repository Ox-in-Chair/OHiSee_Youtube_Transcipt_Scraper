...
class PromptComposer(ttk.Frame):
    ...
    CHIPS = [
        {'id': 'topic', 'prompt': 'What are you researching?', 'type': 'text', 'required': True, 'placeholder': 'e.g., BRCGS automation'},
        {'id': 'audience', 'prompt': 'Who is this for?', 'type': 'text', 'required': False, 'placeholder': 'e.g., food manufacturers'},
        {'id': 'time_window', 'prompt': 'When published?', 'type': 'select', 'options': ['Last week', 'Last month', 'Last 90 days', 'Last year', 'Any time']},
        {'id': 'quality_bar', 'prompt': 'How deep?', 'type': 'select', 'options': ['Quick scan (5 videos)', 'Balanced (15 videos)', 'Deep dive (50 videos)']},
    ...
    def _build_ui(self):
        ...
        # Title
        title = tk.Label(self, text='Research Query Builder',
    ...
    def _render_collapsed(self):
        ...
        # Sentence display
        sentence_frame = tk.Frame(self.content_frame, bg=COLORS['surface'],
                                 bd=1, relief='solid')
        ...
        sentence_label = tk.Label(sentence_frame, text=sentence,
                                 ...
                                 fg=COLORS['text'], wraplength=600, justify='left')
        ...
        for i, chip in enumerate(self.CHIPS):
            ...
            row = i // 2
            col = i % 2
            chip_btn.grid(row=row, column=col, padx=SPACING['xs'],
                         pady=SPACING['xs'], sticky='ew')
    ...
    def _create_chip_button(self, parent, chip: Dict[str, Any]) -> tk.Button:
        ...
        btn = tk.Button(parent, text=display, font=FONTS['body'],
    ...
    def _expand_chip(self, chip_id: str):
        ...
        # Header
        header = tk.Label(self.content_frame, text=chip['prompt'],
        ...
        tk.Button(actions_frame, text='Done', font=FONTS['body'],
                 bg=COLORS['primary'], fg='white', relief='flat',
                 padx=SPACING['md'], pady=SPACING['xs'],
                 command=self._collapse_chip).pack(side='left', padx=SPACING['xs'])

        tk.Button(actions_frame, text='Clear', font=FONTS['body'],
                 bg=COLORS['surface'], fg=COLORS['text'], relief='flat',
                 padx=SPACING['md'], pady=SPACING['xs'],
                 command=lambda: self._clear_chip(chip_id)).pack(side='left')

    def _render_text_input(self, chip: Dict[str, Any]):
        """Render text entry field."""
        entry = tk.Entry(self.content_frame, font=FONTS['body'],
                        bg='white', fg=COLORS['text'], relief='solid', bd=1)
    ...
    def _render_select_input(self, chip: Dict[str, Any]):
        ...
        for option in chip['options']:
            rb = tk.Radiobutton(self.content_frame, text=option, variable=var,
                               value=option, font=FONTS['body'], bg=COLORS['bg'],
                               fg=COLORS['text'])

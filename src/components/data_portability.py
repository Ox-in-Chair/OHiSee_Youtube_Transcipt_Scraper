...
class DataPortability:
    ...
    def export_to_zip(data: Dict[str, Any], output_path: str) -> bool:
        ...
        try:
            with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                ...
                # Export transcripts
                zipf.writestr("transcripts.json", json.dumps(data.get("transcripts", []), indent=2))

                # Export search history
                zipf.writestr("searches.json", json.dumps(data.get("searches", []), indent=2))

                # Export configurations
                zipf.writestr("configs.json", json.dumps(data.get("configs", []), indent=2))
    ...
    def import_from_zip(zip_path: str, merge: bool = True) -> Dict[str, Any]:
        ...
        try:
            imported_data = {"transcripts": [], "searches": [], "configs": []}
            ...
            with zipfile.ZipFile(zip_path, "r") as zipf:
                ...
                if "transcripts.json" in zipf.namelist():
                    imported_data["transcripts"] = json.loads(zipf.read("transcripts.json"))

                # Load searches
                if "searches.json" in zipf.namelist():
                    imported_data["searches"] = json.loads(zipf.read("searches.json"))

                # Load configs
                if "configs.json" in zipf.namelist():
                    imported_data["configs"] = json.loads(zipf.read("configs.json"))
...
class DataPortabilityPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # Header
        tk.Label(
            self, text="Data Portability", font=FONTS["h2"], bg="white", fg=COLORS["text"]
        ).pack(pady=SPACING["md"], anchor="w", padx=SPACING["md"])

        # Export section
        export_frame = tk.Frame(self, bg=COLORS["surface"], bd=1, relief="solid")
        ...
        tk.Label(
            ...
            bg=COLORS["surface"],
            fg=COLORS["text"],
        ).pack(pady=SPACING["sm"], padx=SPACING["md"], anchor="w")
        ...
        tk.Button(
            ...
            cursor="hand2",
        ).pack(pady=SPACING["md"], padx=SPACING["md"], anchor="w")

        # Import section
        import_frame = tk.Frame(self, bg=COLORS["surface"], bd=1, relief="solid")
    ...
    def _handle_export(self):
        ...
        if filename:
            ...
            if success:
                messagebox.showinfo(
                    "Export Successful", f"Research library exported to:\n{filename}"
                )
    ...
    def _handle_import(self):
        ...
        if filename:
            ...
            if imported_data:
                ...
                messagebox.showinfo(
                    "Import Successful", f"Imported {count} transcripts from archive"
                )

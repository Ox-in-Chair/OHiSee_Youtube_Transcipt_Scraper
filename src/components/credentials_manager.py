...
class CredentialsManager(tk.Toplevel):
    ...
    def _build_key_tab(self, parent):
        ...
        desc = tk.Label(
            parent,
            text="Enter your OpenAI API key for query optimization",
            font=FONTS["body"],
            bg="white",
        ...
        # API Key input
        tk.Label(parent, text="API Key:", font=FONTS["body"], bg="white", fg=COLORS["text"]).pack(
    ...
    def _build_model_tab(self, parent):
        ...
        # Model options
        tk.Label(
            parent, text="Select OpenAI Model:", font=FONTS["body"], bg="white", fg=COLORS["text"]
        ...
        for value, label, cost in models:
            ...
            tk.Label(
                frame, text=cost, font=FONTS["small"], bg="white", fg=COLORS["text_secondary"]
        ...
        self.cost_label.pack(anchor="w", padx=SPACING["sm"], pady=(0, SPACING["xs"]))
    ...
    def _build_limits_tab(self, parent):
        ...
        tk.Label(
            parent,
            text="Monthly Spending Cap (USD):",
            font=FONTS["body"],
            bg="white",
            fg=COLORS["text"],
        ).pack(anchor="w", padx=SPACING["md"], pady=(SPACING["md"], SPACING["xs"]))

        self.cap_entry = tk.Entry(
            parent,
            font=FONTS["body"],
            bg=COLORS["surface"],
            fg=COLORS["text"],
        ...
        tk.Label(
            warning_frame,
            text="⚠️ Usage will be tracked locally",
            font=FONTS["body"],
            bg=COLORS["surface"],
            fg=COLORS["warning"],
        ).pack(anchor="w", padx=SPACING["sm"], pady=(SPACING["xs"], 0))
        ...
        tk.Label(
            warning_frame,
            text="The app will stop making API calls when cap is reached.",
            font=FONTS["small"],
            bg=COLORS["surface"],
            fg=COLORS["text_secondary"],
            wraplength=400,
    ...
    def _test_connection(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Connection Failed", f"Could not connect to OpenAI API.\n\n" f"Error: {str(e)}"
            )

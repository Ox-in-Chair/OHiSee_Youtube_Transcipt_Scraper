...
def optimize_search_query(user_input, api_key=None, duration=None, features=None, upload_days=None):
    ...
    try:
        ...
        response = client.chat.completions.create(model="gpt-4", messages=[{"role": "system", "content": GODLY_SEARCH_PROMPT}, {"role": "user", "content": user_input}], max_tokens=80, temperature=0.3)
        optimized = response.choices[0].message.content.strip()
        if optimized.startswith('"') and optimized.endswith('"') and optimized.count('"') == 2: optimized = optimized[1:-1]
